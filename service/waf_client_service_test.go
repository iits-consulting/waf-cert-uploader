...
func TestSetupOtcClientForAkSk(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "", authOptsSlot.(golangsdk.AuthOptions).Username)
	assert.Equal(t, "", authOptsSlot.(golangsdk.AuthOptions).Password)
	assert.Equal(t, "", authOptsSlot.(golangsdk.AuthOptions).DomainName)
	assert.Equal(t, "qwer541235g3", authOptsSlot.(golangsdk.AuthOptions).TenantName)
